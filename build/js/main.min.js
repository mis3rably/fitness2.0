...
var videoButton = document.querySelector('.video-player__play-button');
var tabsButtonList = document.querySelector('.subscription__list');
var initiallyActiveButton = tabsButtonList.querySelector('.subscription__item--active button');
var tabIndicatorFill = document.querySelector('.subscription__indicator-fill');
var tabsBlock = document.querySelector('.subscription__wrapper');
var tabClickableArea = document.querySelector('.faq__tabs-item');
var tabsList = document.querySelector('.faq__tabs-list');
var toggleTabsList = document.querySelector('.faq__buttons-list');
var tabsGroupList = document.querySelectorAll('.faq__tabs-list');
var tabsWrapper = document.querySelector('.faq__tabs-wrapper');
var formMainPage = document.querySelector('.free-visit__form');
var formSubmit = document.querySelector('.free-visit__form button[type="submit"]');
var handleTabButtonClick = function handleTabButtonClick(clickedButton) {
  var buttonWrapper = clickedButton.closest('.faq__buttons-item');
  var tabsType = buttonWrapper.dataset.tabGroupButton;
  var currentActiveButton = toggleTabsList.querySelector('.is-active');
  var currentActiveTabGroup = tabsWrapper.querySelector('.is-shown');
  ...
  for (var i = 0; i < tabsGroupList.length + 1; i++) {
...
if (tabsButtonList && tabIndicatorFill && initiallyActiveButton) {
  tabIndicatorFill.style.width = "".concat(initiallyActiveButton.getBoundingClientRect().width, "px");
  tabIndicatorFill.style.left = "".concat(initiallyActiveButton.offsetLeft, "px");
  tabsButtonList.addEventListener('click', function (evt) {
    if (evt.target.tagName === 'BUTTON' && !evt.target.closest('.subscription__item').classList.contains('subscription__item--active')) {
      var buttonWrapper = evt.target.closest('.subscription__item');
      var tabId = buttonWrapper.dataset.tabButton;
      var activeTabList = tabsBlock.querySelectorAll('.subscription-card__price-wrapper--active');
      var changedTabList = tabsBlock.querySelectorAll(".subscription-card__price-wrapper[data-tab-content=\"".concat(tabId, "\"]"));
      tabsButtonList.querySelector('.subscription__item--active').classList.remove('subscription__item--active');
      tabIndicatorFill.style.width = "".concat(evt.target.getBoundingClientRect().width, "px");
      tabIndicatorFill.style.left = "".concat(evt.target.offsetLeft, "px");
      buttonWrapper.classList.add('subscription__item--active');
      activeTabList.forEach(function (el) {
        el.classList.remove('subscription-card__price-wrapper--active');
      });
      changedTabList.forEach(function (el) {
...
if (tabClickableArea && tabsList && toggleTabsList && tabsGroupList) {
  ...
  tabsWrapper.addEventListener('click', function (evt) {
    var tabElement = evt.target.closest('[data-tab-item="tab"]');
    if (tabElement) {
      var clickedTabButton = tabElement.querySelector('[data-tab-item="button"]');
      clickedTabButton.classList.toggle('tab-button--active');
      tabElement.classList.toggle('is-active');
    }
  });
  toggleTabsList.addEventListener('click', function (evt) {
...
if (formSubmit && formMainPage) {
  formMainPage.addEventListener('submit', function (evt) {
    evt.preventDefault();
